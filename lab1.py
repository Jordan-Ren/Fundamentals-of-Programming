...
class Image:
    ...
    def apply_kernel(self, kernel):
        changed = Image.new(self.width, self.height)
        new_color = 0
        #iterate through each individual pixel
        for x in range(changed.width):
            for y in range(changed.height):
                #for each row/col in the kernel, apply kernel
                for row in range(len(kernel)):
                    for col in range(len(kernel)):
                        new_color += self.get_pixel((x-(len(kernel) - 1)//2 + col), (y-(len(kernel) - 1)//2 + row)) * kernel[row][col]
                changed.set_pixel(x,y,round(new_color))
                new_color = 0
        return changed

    def final_clipper(self):
        for x in range(self.width):
            for y in range(self.height):
                #issues debugging, ask LA
                pix = self.get_pixel(x, y)
                self.set_pixel(x,y,round(pix))
                if pix > 255:
    ...
    def inverted(self):
        return self.apply_per_pixel(lambda color: 255 - color)

    def blurred(self, n):
        kernel = []
        val = 1/(n**2)
        row = []
        #creates kernel with appropriate vals
        for i in range(n):
            for j in range(n):
                row.append(val)
            kernel.append(row)
            row = []
        changed = Image.apply_kernel(self, kernel)
        changed.final_clipper()
        return changed
    ...
    def sharpened(self, n):
        ...
        for i in range(n):
            ...
            kernel.append(row)
            row = []
        #one comprehension, regualr image has 1 in center of kernelself.
        #so 2 - val in center, all other coords are -val
        kernel[(n-1)//2][(n-1)//2] += 2
        changed = Image.apply_kernel(self, kernel)
        changed.final_clipper()
        return changed
    ...
    def edges(self):
        ...
        Ox = self.apply_kernel(Kx)
        Oy = self.apply_kernel(Ky)
        result = Image.new(self.width,self.height)
        for x in range(result.width):
            for y in range(result.height):
                new_val = (Ox.get_pixel(x,y)**2+Oy.get_pixel(x,y)**2)**.5
                result.set_pixel(x,y, new_val)
        result.final_clipper()
        return result
...
if __name__ == '__main__':
    ...
    # properly, whether we're runninsg interactively or not:
